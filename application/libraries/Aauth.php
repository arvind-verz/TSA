...
class Aauth
{
    ...
    public function login($identifier, $pass, $remember = false, $totp_code = null)
    {
        ...
        $query = $this->aauth_db->where('is_archive', 0);
    ...
    public function permission_access($perm_id, $module, $views, $creates, $edits, $deletes)
    {
        $data = array(
            'perm_id' => $perm_id,
            'module'  => $module,
        ...
        $this->aauth_db->insert($this->config_vars['perms_access'], $data);
        return true;
    }

    public function update_permission_access($perm_id, $module, $views, $creates, $edits, $deletes)
    {
        $data = array(
            'views'   => $views,
            'creates' => $creates,
        ...
        $this->aauth_db->where(['perm_id' => $perm_id, 'module' => $module]);
        $this->aauth_db->update($this->config_vars['perms_access'], $data);
        return true;
    }
    ...
    public function is_group_allowed($perm_par, $group_par = false)
    {
        ...
        if ($group_par != false) {
            ...
            if (strcasecmp($group_par, $this->config_vars['admin_group']) == 0) {return true;}
    ...
    public function get_user_var($key, $user_id = false)
    {
        ...
        // if variable not set
        if ($query->num_rows() < 1) {return false;} else {
