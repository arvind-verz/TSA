<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class Product_model extends CI_Model
    {
		function get_product_detail($seo_url) {		
					 ...
					 $this->db->select('p.*,c.cat_name,c.image_banner,c.parent_id,c.seo_url,p.seo_url p_seo_url')
					 ->from(TBL_TOOLS_PRODUCTS . ' as `p`')
					 ->join(TBL_TOOLS_CATEGORIES . ' as `c`' , 'p.cat_id = c.cat_id')
					 ->where('p.status', 'Y')
		...
		function get_product_video($products_id) {
