<?php
    ...
    class Tools_category_model extends CI_Model
    {
		 ...
		 function get_sub_category_details($cat_id) {		
			...
			function count_subcat($cat_id) {
                  $this->db->select('count(cat_id) as `TotalNum`')
                          ->from(TBL_CATEGORIES)
						  ->where('status', 'Y')
					 	  ->where('parent_id', $cat_id);
		  ...
		  function get_subcat($cat_id, $total_row_display , $limit_from) {		
