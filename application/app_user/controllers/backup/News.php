...
class News extends MY_Controller {
	...
	public function news_all() {
		...
		$data_msg['page'] = $page = $this->Cms_model->get_page('news');
				
		$data_msg['url'] = 'news';
		
		$data_msg['menu_id'] = $page[0]['menu_id'];
		
		$data_msg['url_name'] = 'news';

      $news_per_page = $this->all_function->get_site_options('news_per_page');
		...
		$data_msg['pagi'] = $this->admin_pagination->ajaxpagination($total_items, $news_per_page, $current_page, 3, base_url('news'));		
	...
	public function news_list($month,$year) {
		...
		$data_msg['pagi'] = $this->admin_pagination->ajaxpagination($total_items, $products_per_page, $current_page, 3,base_url('news/'.$month.'/'.$year));			
	...
	public function news_details($seo_url) {
		...
		if(count($page)>0){
			...
			$menu = $this->Cms_model->get_page('news');
			$data_msg['page1']=$menu;
