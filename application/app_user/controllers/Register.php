...
class Register extends MY_Controller {
	...
	function register_details($seo_url)
	{
	...
	function seat_limit_checking_from($id,$member_type_id){
		$limit_checking = $this->Register_model->get_events_reg_limit_checking($member_type_id,$id);
		$reg_this_member_type = $this->Register_model->count_reg_this_member_type($member_type_id,$id);
	...
	function max_no_reg_per_company($id,$member_type_id,$member_id){
		if($member_type_id==4 || $member_type_id==5){
			...
			$reg_per_company = $this->Register_model->get_reg_per_company($member_type_id,$id);
			$reg_this_company = $this->Register_model->count_reg_this_company($member_type_id,$id,$member_id);
	...
	function set_event_price($id,$member_type_id){
		$price = 0.00;
		$istevent_reg = $this->Register_model->count_register($member_type_id,$id);
		if($istevent_reg==0){
			$price = $this->Register_model->get_event_price1($id,$member_type_id);
		}else{
			$price = $this->Register_model->get_event_price2($id,$member_type_id);
		}	
		return $price;	
	}
	
	function set_event_price_an($event_id,$member_type_id,$member_id){
		$price = 0.00;
		#$istevent_reg = $this->Register_model->count_register($member_type_id,$id);
		$istevent_reg = $this->Register_model->count_reg_this_company($member_type_id,$event_id,$member_id);
		//echo $this->db->last_query();exit;
		if($istevent_reg==0){
			$price = $this->Register_model->get_event_price1($event_id,$member_type_id);
		}else{
			$price = $this->Register_model->get_event_price2($event_id,$member_type_id);
		}	
		return $price;	
	}
	
	function checking_payment_type($id,$member_type_id){
		return $payment_type = $this->Register_model->get_events_reg_payment_type($member_type_id,$id);
	}	
	
	public function ajax_promo_code($event_id) {
		if(isset($_POST['promo_code'])){
			$promo_code = $_POST['promo_code'];
			$code = $this->Register_model->get_event_promo_code_details($event_id);
			if($code == $promo_code){
				$member_type_id = 4;				
	...
	function register_form_member($seo_url){
		...
		$data_msg['price'] = $price = $this->set_event_price_an($event_id,$member_type_id,$member_id);
		
		$istevent_reg = $this->Register_model->count_reg_this_company($member_type_id,$event_id,$member_id);
		$price_1 = $this->Register_model->get_event_price1($event_id,$member_type_id);
		$price_2 = $this->Register_model->get_event_price2($event_id,$member_type_id);
		if($istevent_reg!=0 && $price_1!=$price_2){
			
			//$data_msg['custom_msg'] = "<p>The 1st complimentary seat has been taken. Please proceed to register at the Member's rate.</p>";
			$data_msg['custom_msg'] = $this->all_function->get_site_options('company_pax_msg');
		}
		...
		if($_POST){
			if ($this->form_validation->run() == FALSE)
			{
				...
				$price = $this->set_event_price_an($event_id,$member_type_id,$member_id);
				...
				if($price!=$ShowPrice){
					$data_msg['error_msg'] = '<p>Kindly note the complimentary seat for your company has already been taken.</p>';
					$this->view('register_form_member',$data_msg);
				}else{
					...
					$event_reg_id = $this->Register_model->add_event_registration($data);					
					$this->invoice_create($event_reg_id);
					if($price==0 || $payment_type=='Offline'){
						$this->registration_email_member($event_reg_id);
					}					
					if($price>0 && $payment_type=='Paypal'){
						...
						/*if($_SERVER['REQUEST_METHOD']=="POST"){
							header("Location: " . $_SERVER['REQUEST_URI'].'?sussecc=ok', true, 303);
   							exit();
	...
	function register_form($seo_url){
		...
		$configForm = array(
                 array(
				  ...
				  array(
                     'field'   => 'email',
                     'label'   => 'Email',
                     'rules'   => 'trim|valid_email|required|callback_edit_unique['.TBL_EVENT_REGISTRATION.'.email.'.$event_id.'.'.$payment_type.']'
                  ),
				  array(
                     'field'   => 'phone_no',
                     'label'   => 'Contact Number',
		...
		if($_POST){
			if ($this->form_validation->run() == FALSE)
			{
				...
				if($price!=$ShowPrice){
					$error_msg = '<p>Kindly note the complimentary seat for your company has already been taken.</p>';
					$this->session->set_flashdata('error_status', 1);
					$this->session->set_userdata(array('msg_type' =>0,'error_msg' => $error_msg));
					$this->view('register_form',$data_msg);
				}else{
					...
					$this->invoice_create($event_reg_id);
					if($price==0 || $payment_type=='Offline'){
						$this->registration_email($event_reg_id);
					}
					if($price>0 && $payment_type=='Paypal'){
						$data['event_reg_id']=$event_reg_id;
						$this->load->view('paypal_view',$data);
	...
	function registration_email($event_reg_id){
		
		$details=$this->Register_model->get_event_registration_detail($event_reg_id);
		$full_name=$details['first_name'].' '.$details['last_name']; 
		$event_id=$details['event_id'];
		$event=$this->Register_model->get_event_details_by_id($event_id);
		...
		$body			  = str_replace("{DATE}", date("F d, Y", strtotime($event['start_date'])), $body);
		$body			  = str_replace("{TIME}", $event['event_time'], $body);
		$body			  = str_replace("{VENUE}", $event['venue'], $body);
		...
		$body_auto			  = str_replace("{EVENT_NAME}", $event['title'], $body_auto);
		$body_auto			  = str_replace("{DATE}", date("F d, Y", strtotime($event['start_date'])), $body_auto);
		$body_auto			  = str_replace("{TIME}", $event['event_time'], $body_auto);
		$body_auto			  = str_replace("{VENUE}", $event['venue'], $body_auto);
		...
		$this->all_function->batch_email_attach($details['company_email'],$auto_from, $auto_from_name,$user_bcc, $auto_subject, $body_auto,$fullPathInvoice);
	}
	
	
	public function registration_email_member($event_reg_id)
	{
		    $user_bcc='';
			$details = $this->Register_model->get_event_registration_detail($event_reg_id);
			$full_name = $details['first_name'].' '.$details['last_name'];
	...
	public function invoice_create($event_reg_id)
	{
	    $str11='';
			...
			if($details['payment_type']=='Offline'){$str11='Payable';}else{$str11='Paid';}
...
<body style="margin:0px; padding:0px; font-family:Arial, Helvetica, sans-serif; font-size:10px; color:#393a42;">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tbody>
    ...
    <tr>
      <td align="center" valign="top"><table width="250" border="0" align="center" cellpadding="0" cellspacing="0">
  <tbody>
    <tr>
      <td align="center" valign="top"><img src="assets/pdf/logo.png"  width="250" alt=""></td>
    </tr>
  </tbody>
   </table>
	  </td>
    ...
    <tr>
      <td align="center" valign="top" style="font-size:30px; text-transform:uppercase; font-weight:bold;">INVOICE</td>
    </tr>
    <tr>
    ...
    <tr>
      <td align="left" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tbody>
          <tr>
            <td align="left" valign="top" style="padding:10px; background:#f5f5f5;  line-height:20px;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tbody>
                <tr>
                  <td width="29%" align="left" valign="top" style="font-weight:bold;" >Event Title</td>
                  <td width="2%" align="left" valign="top">:</td>
                  <td width="69%" align="left" valign="top">'.$event_details['title'].'</td>
                </tr>
                <tr>
                  <td align="left" valign="top" style="font-weight:bold;" >Date</td>
                  <td align="left" valign="top">:</td>
                  <td align="left" valign="top">'.date("F d, Y", strtotime($event_details['start_date'])).'</td>
                </tr>
                <tr>
                  <td align="left" valign="top" style="font-weight:bold;">Time</td>
                  <td align="left" valign="top">:</td>
                  <td align="left" valign="top">'.$event_details['event_time'].'</td>
                </tr>
                <tr>
                  <td align="left" valign="top" style="font-weight:bold;" >Venue</td>
                  <td align="left" valign="top">:</td>
            ...
            <td align="left" valign="top" style="padding:10px; background:#f5f5f5;  line-height:20px;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tbody>
               <tr>
                  ...
                  <td width="29%" align="left" valign="top" style="font-weight:bold;">Company</td>

                  <td width="2%" align="left" valign="top">:</td>

                  <td width="69%" align="left" valign="top">'.$details['company_name'].'</td>

                </tr>
                <tr>
                  <td width="29%" align="left" valign="top" style="font-weight:bold;">First Name</td>
                  <td width="2%" align="left" valign="top">:</td>
                  <td width="69%" align="left" valign="top">'.$details['first_name'].'</td>
                </tr>
                <tr>
                  <td align="left" valign="top" style="font-weight:bold;">Last Name</td>
                  <td align="left" valign="top">:</td>
                  <td align="left" valign="top">'.$details['last_name'].'</td>
                </tr>
				<tr>
                  <td align="left" valign="top" style="font-weight:bold;">Participant Email</td>
                  <td align="left" valign="top">:</td>
                  <td align="left" valign="top">'.$details['email'].'</td>
                </tr>
				<tr>
                  <td align="left" valign="top" style="font-weight:bold;">Additional Comments</td>
                  <td align="left" valign="top">:</td>
  ...
  <tr>
       ...
       <tr>
            <td align="left" valign="top" style="padding:10px; background:#f5f5f5;  line-height:20px;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tbody>
                <tr>
                  <td width="29%" align="left" valign="top" style="font-weight:bold;">Amount '.$str11.'</td>
                  <td width="2%" align="left" valign="top">:</td>
                  <td width="69%" align="left" valign="top">SGD'.$details['price'].'</td>
                </tr>
                <tr>
                  <td align="left" valign="top" style="font-weight:bold;">Payment Method</td>
                  <td align="left" valign="top">:</td>
                  <td align="left" valign="top">'.$details['payment_type'].'</td>
                </tr>
                <tr>
                  <td align="left" valign="top" style="font-weight:bold;">Billing Address</td>
                  <td align="left" valign="top">:</td>
                  <td align="left" valign="top">'.$details['billing_address'].'</td>
                </tr>';
				if($details['payment_type']=='Offline'){
                $html .='<tr>
                  <td align="left" valign="top" style="font-weight:bold;">Offline Payment Comments</td>
                  <td align="left" valign="top">:</td>
		  ...
		  if($details['payment_type']=='Offline'){
			  $html .='</tr>
  <tr>
            <td align="left" valign="top" style="padding:0 10px; font-size:18px;"><strong>Bank Details</strong></td>
          </tr>
          <tr>
            <td align="left" valign="top">&nbsp;</td>
          </tr>
       <tr>
            <td align="left" valign="top" style="padding:10px; background:#f5f5f5;  line-height:20px;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tbody>
                <tr>
                  <td width="29%" align="left" valign="top" style="font-weight:bold;">Payment mode</td>
                  <td width="2%" align="left" valign="top">:</td>
                  <td width="69%" align="left" valign="top"> By Cheque or Telegraphic transfer (net of all bank charges)<br/>
				  Bank :DBS Bank Limited Singapore, Raffles Place Branch<br/>
				  22 Malacca Street, #01-00 Royal Brothers Building Singapore 048980<br/>
				  A/C: 022-013808-1  Bank Code : 7171<br/>
				  Branch code : 022  Swift : DBSSSGSG
				  </td>
                </tr>
                <tr>
                  <td align="left" valign="top" style="font-weight:bold;">Name of Payee</td>
                  <td align="left" valign="top">:</td>
                  <td align="left" valign="top">Singapore Venture Capital & Private Equity Association</td>
                </tr>
              </tbody>
            </table></td>
          </tr>
    ...
    <tr>
	  ...
	  <div style="border-top:5px solid #ed1b24; padding:5px 0; font-size:8px; width:100%; background:#FFF; position:fixed; bottom:0px;
      margin-bottom:0px;">
	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tbody>
          <tr>
...
';
			...
			$this->Register_model->update_event_registration($data,$event_reg_id);
	}
	
	public function registration_email_paypal($event_reg_id,$order_transaction_id)
	{
		$details=$this->Register_model->get_event_registration_detail($event_reg_id);
		$full_name = $details['first_name'].' '.$details['last_name'];
		$event_details = $this->Events_model->get_event_details_by_id($details['event_id']);
		$transaction_details = $this->Register_model->get_transaction_details_by_id($order_transaction_id);
	...
	public function edit_unique($value, $params)
	{
        ...
        list($table, $field, $current_id,$payment_type) = explode(".", $params);
		/*if($payment_type=='Paypal')
		{
		$query = $this->db->select()->from($table)->where($field, $value)->where('event_id', $current_id)->where('is_paid','Paid')->get()->result_array();
		}else{
		//$query = $this->db->select()->from($table)->where($field, $value)->where('event_id', $current_id)->get()->result_array();
		$query = $this->db->select()->from($table)->where($field, $value)->where('event_id', $current_id)->where('payment_type','Offline')->where("(booking_status != 'R' )",NULL, FALSE)->get()->result_array();
		}
		...
		// Raj
		$query1 = array(); $query2 = array();
		$query1 = $this->db->select()->from($table)
			->where($field, $value)
			->where('event_id', $current_id)
			->where('payment_type','Paypal')
			->where('is_paid','Paid')
			->where_in('booking_status',array('N','Y'))
			->get()->result_array();	
		$query2 = $this->db->select()->from($table)
			->where($field, $value)
			->where('event_id', $current_id)
