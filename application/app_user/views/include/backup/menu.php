<nav id="smoothmenu1" class="ddsmoothmenu clear-fix">
  <ul>
    ...
    <?php if(($val['link_type']=='external' && $val['external_url']=='##' ) || ($val['menu_title']=='Products')){?>
    <li <?php if($cat_id>0){echo 'class="Select"';}?> ><a href="javascript:void(0);"><?php echo $val['menu_title'];?></a>
      <ul>
        <?php $ProductCat = $this->All_function_model->get_root_categories();?>
        <?php foreach ($ProductCat as $keyca => $cat){
			$parent_id = $this->All_function_model->get_parent_categories_id($cat_id);
			?>
        <li <?php if($parent_id==$cat['cat_id']){echo 'class="Select"';}?>> <a href="javascript:void(0);"><?php echo $cat['cat_name'];?></a>
          <?php $SubCat = $this->All_function_model->get_subcategories($cat['cat_id']);?>
          <?php if(count($SubCat)>0){?>
          <ul>
            <?php foreach ($SubCat as $keycat => $sub){?>
            <li <?php if($url_name==$sub['seo_url'] || $cat_id==$sub['cat_id']){echo 'class="Select"';}?>> <a href="<?php echo base_url('category/'.$sub['seo_url']); ?>"><?php echo $sub['cat_name'];?></a> </li>
            <?php } ?>
          </ul>
          <?php }?>
        </li>
