...
class ToolsContact extends MY_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Tools_contact_model', '', TRUE);

    }
	public function manage_tools_contact() {
		...
		if(isset($_POST['OkFilter'])){
			...
			$data_msg['total'] = $total_items = $this->Tools_contact_model->get_contact_num_filter($FlterData);			
			$data_msg['pagi'] = $this->admin_pagination->ajaxpagination($total_items, $items_per_page, $current_page, 3,base_url('manage-tools-contact'));
			$data_msg['display_result'] = $this->Tools_contact_model->get_all_contact_filter($items_per_page, $limit_from, $FlterData)->result_array();
		...
		elseif( isset($_POST['action']) && $_POST['action']=='Delete'){				
			...
			if(count($checkbox_arr) !== 0){
			  foreach ($checkbox_arr as $del_id){
				 $this->Tools_contact_model->delete_contact($del_id);
			  }
			  $this->session->set_flashdata('success_msg', 'Record has been deleted successfully');
			  redirect(base_url('manage-tools-contact'));
			  exit;
			  }else{redirect(base_url('manage-tools-contact'));}			
		}
		elseif($this->session->userdata('FlterData')){
		 ...
		 $this->session->set_userdata(array('FlterData' => '' ));					
		  $FlterData = array(
        	...
        	$total_items = $this->Tools_contact_model->get_contact_num();
        	$data_msg['pagi'] = $this->admin_pagination->ajaxpagination($total_items, $items_per_page, $current_page, 3,base_url('manage-tools-contact'));
        	$get_value = $this->Tools_contact_model->get_all_contact($items_per_page, $limit_from);
        ...
        $this->view('tools_contact/manage_contact', $data_msg);
		}
		if($permission=='N'){			
	...
	public function view_tools_contact($id) {
        ...
        $get_value = $this->Tools_contact_model->get_contact_details($id);
		...
		$this->Tools_contact_model->update_contact($data, $id);

        $this->view('tools_contact/view_contact', $data_msg);
	...
	public function del_tools_contact($id) {
        ...
        $data_msg = array();
          ...
          $this->Tools_contact_model->delete_contact($id);
	...
	public function export_tools_contact($pid=0) {		
        $data_msg = array();		        
		...
		//for($i=3;$i<=count($get_value)+2;$i++){
			...
			$country_name=$this->Tools_contact_model->get_country_name($val['contact_country_id']);
