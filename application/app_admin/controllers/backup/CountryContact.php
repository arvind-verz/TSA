...
class CountryContact extends MY_Controller {

    function __construct() {		
        parent::__construct();
        $this->load->model('Country_contact_model', '', TRUE);
    }
	
	 public function manage_country_contact() {
		...
		if(isset($_POST['OkFilter'])){
			...
			$data_msg['total'] = $total_items = $this->Country_contact_model->count_country_filter($FlterData);			
			$data_msg['pagi'] = $this->admin_pagination->ajaxpagination($total_items, $items_per_page, $current_page, 3,base_url('manage-country-contact'));
			$data_msg['display_result'] = $this->Country_contact_model->get_country_filter($items_per_page, $limit_from, $FlterData)->result_array();
		}
		elseif( isset($_POST['action']) && $_POST['action']=='Delete'){				
			...
			if(count($checkbox_arr) !== 0){
			  foreach ($checkbox_arr as $del_id){
				...
				$this->Country_contact_model->del_country($del_id);	
			  }
			  $this->session->set_flashdata('success_msg', 'Record has been deleted successfully');
			  redirect(base_url('manage-country-contact'));
			  exit;
			  }else{redirect(base_url('manage-country-contact'));exit;}			
		}
		elseif( isset($_POST['action']) && $_POST['action']=='Enable'){				
			...
			if(count($checkbox_arr) !== 0){
			  foreach ($checkbox_arr as $del_id){
				  $data = array('status' => 'Y');
                  $this->Country_contact_model->update_country($data,  $del_id);
			  }
		...
		elseif($this->session->userdata('FlterData')){
		 ...
		 $this->session->set_userdata(array('FlterData' => '' ));					
		  $FlterData = array(
        	...
        	$total_items = $this->Country_contact_model->count_country();
        	$data_msg['pagi'] = $this->admin_pagination->ajaxpagination($total_items, $items_per_page, $current_page, 3,base_url('manage-country-contact'));
        	$get_value = $this->Country_contact_model->get_country($items_per_page, $limit_from);
       		$data_msg['display_result'] = $get_value->result_array();
		}
		
        $this->view('country_contact/manage_country', $data_msg);
    }
	public function add_country_contact() {
		...
		if($_POST){	
			if ($this->form_validation->run() == FALSE)
			{
			  ...
			  $data = array(					
                    ...
                    $page_id = $this->Country_contact_model->add_country($data);
	...
	public function del_country_contact($id) {
		
        $data_msg = array();
		
		$this->Country_contact_model->del_country($id);			
	...
	public function edit_country_contact($id){
		...
		$get_result = $this->Country_contact_model->get_country_details($id);
		...
		if($_POST){	
			...
			{	
             ...
             $data = array(					
				...
				$this->Country_contact_model->update_country($data,  $id);
                $this->session->set_flashdata('success_msg', 'Successfully updated.');				
				redirect(base_url("edit-country-contact/$id")); 
