...
class Categories extends MY_Controller {
    ...
    public function manage_categories() {
		...
		if(isset($_POST['OkFilter'])){
			...
			$data_msg['total'] = $total_items = $this->Categories_model->count_categories_filter($FlterData);			
			$data_msg['pagi'] = $this->admin_pagination->ajaxpagination($total_items, $items_per_page, $current_page, 3,base_url('manage-categories'));
			$data_msg['display_result'] = $this->Categories_model->get_categories_filter($items_per_page, $limit_from, $FlterData)->result_array();
		}
		elseif( isset($_POST['action']) && $_POST['action']=='Delete'){				
			$checkbox_arr = $this->input->post('id');
			if(count($checkbox_arr) !== 0){
			  foreach ($checkbox_arr as $del_id){
				$get_result = $this->Categories_model->get_categories_details($del_id);				
				$details = $get_result->result_array();				
				$file = MAIN_SITE_AB_UPLOAD_PATH.'categories/original/'.$details[0]['image_name'];
				if(is_file($file)){unlink($file); } 
				$file = MAIN_SITE_AB_UPLOAD_PATH.'categories/thumb/'.$details[0]['image_name'];
				if(is_file($file)){unlink($file);} 				  
				$this->Categories_model->del_categories($del_id);
			  }
			  $this->session->set_flashdata('success_msg', 'Record has been deleted successfully');
			  redirect(base_url('manage-categories'));
			  exit;
			  }else{redirect(base_url('manage-categories'));}			
		}
		elseif( isset($_POST['action']) && $_POST['action']=='SortOrder'){				
			...
			foreach ($checkbox_arr as $del_id){
			  ...
			  $this->Categories_model->update_categories($data,  $del_id);
			  $c++;
			}
		...
		elseif($this->session->userdata('FlterData')){
		 ...
		 $this->session->set_userdata(array('FlterData' => '' ));					
		  $FlterData = array(
        	...
        	$total_items = $this->Categories_model->count_categories();
        	$data_msg['pagi'] = $this->admin_pagination->ajaxpagination($total_items, $items_per_page, $current_page, 3,base_url('manage-categories'));
        	$get_value = $this->Categories_model->get_categories($items_per_page, $limit_from);
       		$data_msg['display_result'] = $get_value->result_array();
		}
		
        $this->view('categories/manage_categories', $data_msg);
    } 
	...
	public function add_categories() {
		...
		$config = array(
               array(
				...
				array(
                     'field'   => 'seo_url',
                     'label'   => 'Url',
                     'rules'   => 'trim|required|is_unique['.TBL_CATEGORIES.'.seo_url]'
                  ),
				array(
                     'field'   => 'parent_id',
		...
		if($_POST){				
			if ($this->form_validation->run() == FALSE)
			{
				...
				$this->view('categories/add_categories', $data_msg);
			}else{
            $post_data = $_POST; $error_msg = '';$im_nm='';$image_banner_name='';$flag=0;
				  ...
				  if ($this->all_function->check_image_valid($_FILES['image_banner']['tmp_name'])!=1){
					 $error_msg .= ' Invalid Banner Image.';
					   ...
					   $config = array(
						...
						'temp' => 'temp',
						'resize' => array(
						array('height' => 251, 'width' => 1400, 'save' => 'categories/banner/original/'),
						array('height' => 45, 'width' => 200, 'save' => 'categories/banner/thumb/')
						)
						);
	...
	public function edit_categories($cat_id) {
		
        $data_msg = array();

        $get_result = $this->Categories_model->get_categories_details($cat_id);
		...
		$config = array(
               array(
				...
				array(
                     'field'   => 'seo_url',
                     'label'   => 'Url',
                     'rules'   => 'trim|required|callback_edit_unique['.TBL_CATEGORIES.'.seo_url.'.$cat_id.']'
                  ),
				array(
                     'field'   => 'parent_id',
                     'label'   => 'Parent Category',
		...
		if($_POST){				
			...
			{
				$data_msg['error_msg'] = $error_msg = strip_tags(validation_errors());
				$this->view('categories/edit_categories', $data_msg);
			}else{
			...
			if($_FILES['image_banner']['tmp_name']!=''){
				  if ($this->all_function->check_image_valid($_FILES['image_banner']['tmp_name'])!=1){
					  $error_msg .= ' Invalid Banner Image.';  
					    ...
					    $file = MAIN_SITE_AB_UPLOAD_PATH.'categories/banner/original/'.$details[0]['image_banner'];
						if(is_file($file)){
						unlink($file); 
						}
						$file = MAIN_SITE_AB_UPLOAD_PATH.'categories/banner/thumb/'.$details[0]['image_banner'];
						if(is_file($file)){
						unlink($file); 
						}
            ...
            if ($error_msg != '') { 
                ...
                $this->view('categories/edit_categories', $data_msg);
                    ...
                    $this->Categories_model->update_categories($data,  $cat_id);

                $this->session->set_flashdata('success_msg', 'Successfully updated.');
				redirect(base_url("edit-categories/$cat_id"));
            }
        }
		}else{
			$this->view('categories/edit_categories', $data_msg);
	...
	public function del_categories($cat_id) {
		...
		$this->Categories_model->del_categories($cat_id);
