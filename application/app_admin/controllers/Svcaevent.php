...
class Svcaevent extends MY_Controller {
	...
	function svcareport($id)
	{
		...
		/*SELECT member_type_id, COUNT(*) FROM svca_event_registration GROUP BY member_type_id*/
			
			$this->db->select('count(*) as `total`,member_type_id')
                          ->from(TBL_EVENT_REGISTRATION)
						  ->where('event_id', $id)
						  ->where('booking_status != "R"')
						  ->group_by('member_type_id')
			...
			{
				if(isset($val['member_type_id']) && $val['member_type_id']==1)
				$data_msg['full']=$val['total'];
				
				if(isset($val['member_type_id']) && $val['member_type_id']==2)
				$data_msg['asso']=$val['total'];
				
				if(isset($val['member_type_id']) && $val['member_type_id']==3)
				$data_msg['indi']=$val['total'];
				
				if(isset($val['member_type_id']) && $val['member_type_id']==4)
				$data_msg['partner']=$val['total'];
				
				if(isset($val['member_type_id']) && $val['member_type_id']==5)
				$data_msg['visitor']=$val['total'];
		    ...
		    $details = $get_result->result_array();
			$data_msg['details']=$details;
			$this->view('svcaevent/report_view', $data_msg);
