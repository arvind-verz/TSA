	...
	function count_products() {
	 ...
	 function get_products_details($id) {
		...
		function update_page_cms($data , $id) {
				  ...
				  $this->db->where('id' , $id);
                  $this->db->update(TBL_TOOLS_PRODUCTS , $data);
          }
		 /*function add_products_attributes($data){
		  ...
		  function add_products($data){
              $this->db->insert(TBL_TOOLS_PRODUCTS , $data);
			  return $this->db->insert_id();
		  }
		   function del_products($id){
			  $this->db->where('id' , $id);
              $this->db->delete(TBL_TOOLS_PRODUCTS);
		  }
		 function add_additional_images_file($data){
              $this->db->insert(TBL_TOOLS_PRODUCTS_IMAGES , $data);
		  ...
		  function delete_products_file($id){
		  			$this->db->where('id' , $id)
                          ->delete(TBL_TOOLS_PRODUCTS_IMAGES);
		  }
...
########video###########		 
		 function get_video_num($product_id) {
                  $this->db->select('count(id) as `TotalNum`')
                          ->from(TBL_TOOLS_PRODUCTS_VIDEO)
		...
		function get_all_video($product_id,$total_row_display , $limit_from) {
                  $this->db->select('*')
                          ->from(TBL_TOOLS_PRODUCTS_VIDEO)
						  ->where('products_id' , $product_id)
		  ...
		  function add_video($data){
              $this->db->insert(TBL_TOOLS_PRODUCTS_VIDEO , $data);
		  } 
		  ...
		  function update_video($data , $video_id) {
                  $this->db->where('id' , $video_id);
                  $this->db->update(TBL_TOOLS_PRODUCTS_VIDEO , $data);
          }   
		 
		 function delete_video($video_id) {
                  $this->db->where('id' , $video_id)
                          ->delete(TBL_TOOLS_PRODUCTS_VIDEO);
          }
		  
		  
		  function get_video_num_filter($product_id,$FlterData) {
                  $this->db->select('count(id) as `TotalNum`')
                          ->from(TBL_TOOLS_PRODUCTS_VIDEO)
						  ->where('products_id' , $product_id);
				  if( count($FlterData) > 0 ){					 
		  ...
		  function get_all_video_filter($product_id,$total_row_display , $limit_from,$FlterData) {
                  $this->db->select('*')
                          ->from(TBL_TOOLS_PRODUCTS_VIDEO);
				  if( count($FlterData) > 0 ){					 
					foreach($FlterData as $key=>$value){				
	...
	#####################
		...
		function del_product_pdf($id) {
                  $this->db->where('products_id' , $id)
                          ->delete(TBL_TOOLS_PRODUCTS_PDF);
          }   
